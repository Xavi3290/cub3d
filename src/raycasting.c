...
/*   Updated: 2024/11/26 12:40:05 by cgaratej         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void perform_dda(t_ray *ray, t_game *game) {
    int hit = 0;
    int max_steps = game->mapinfo.width * game->mapinfo.height;
    int steps = 0;
    ...
    while (hit == 0 && steps < max_steps) {
        ...
        if (ray->mapX < 0 || ray->mapX >= game->mapinfo.width || ray->mapY < 0 || ray->mapY >= game->mapinfo.height) {
            hit = 1;
            break;
        }
